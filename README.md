...
uv run python -m massgen.cli --config massgen/configs/gpt5.yaml "what's best to do in Stockholm in October 2025"

uv run python -m massgen.cli --config massgen/configs/gpt5.yaml "give me all the talks on agent frameworks in Berkeley Agentic AI Summit 2025, note, the sources must include the word Berkeley, don't include talks from any other agentic AI summits"
```
...
uv run python -m massgen.cli --config massgen/configs/gpt5.yaml "Write a short story about a robot who discovers music."
```

### 3. 🧠 Research
```bash
uv run python -m massgen.cli --config massgen/configs/gpt5.yaml "How much does it cost to run HLE benchmark with Grok-4"
```
