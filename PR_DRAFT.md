...
- **`orchestrator.py`: 21,599 → 9,319 lines (−12,280, 57% reduction)** — 42 collaborator classes extracted into a new `massgen/orchestrator_collaborators/` package. The Orchestrator class keeps thin delegator methods so every existing call site (internal and external) works unchanged.
