...
- **`orchestrator.py`: 21,599 → 8,847 lines (−12,752, 59% reduction)** — 46 collaborator classes extracted into a new `massgen/orchestrator_collaborators/` package. The Orchestrator class keeps thin delegator methods so every existing call site (internal and external) works unchanged.
