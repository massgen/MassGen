...
### v0.1.97 - Latest
**Application-Layer Permission Engine:** Opt-in `allow/ask/deny` rules + risk-tiered approval (modal / automation policy / file), audit ledger, per-agent roles, guardrail prompt — the app-layer companion to v0.1.96's OS sandbox
...
- **Approval that fits the run**: interactive **modal** (allow once/session/always · reject), automation **policy** (`risk-based`/`deny-all`/`allow-all`), or **file** request/response handshake for headless/remote — fail-closed by design
...
# Interactive: a high-risk command pops the approval modal (allow once/session/always · reject)
