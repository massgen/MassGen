...
class ResponseBackend(LLMBackend):
    ...
    def convert_messages_to_response_api_format(
        ...
        """
        import json
        print(f"\n[DEBUG] ===== convert_messages_to_response_api_format START =====")
        print(f"[DEBUG] Input messages count: {len(messages)}")
        print(f"[DEBUG] Full input messages: {json.dumps(messages, indent=2)}")
        
        # First pass: remove 'status' from messages without 'role'
        print(f"\n[DEBUG] ===== First pass: removing 'status' from messages without 'role' =====")
        cleaned_messages = []
        for i, message in enumerate(messages):
            if 'status' in message and 'role' not in message:
                print(f"[DEBUG] Message {i}: Found 'status' in message without 'role', removing it")
                print(f"[DEBUG] Original message: {json.dumps(message, indent=2)}")
                # Create a copy without 'status'
                cleaned_message = {k: v for k, v in message.items() if k != 'status'}
                print(f"[DEBUG] Cleaned message: {json.dumps(cleaned_message, indent=2)}")
        ...
        for i, message in enumerate(cleaned_messages):
            print(f"\n[DEBUG] ----- Processing message index {i} -----")
            print(f"[DEBUG] Message keys: {list(message.keys())}")
            print(f"[DEBUG] Full message: {json.dumps(message, indent=2)}")
            
            # Check if 'status' field exists
            if 'status' in message:
                print(f"[DEBUG] ⚠️  WARNING: Found 'status' field in message {i} with value: {message['status']}")
            if message.get("role") == "tool":
                print(f"[DEBUG] Message type: tool message (Chat Completions format)")
                ...
                print(f"[DEBUG] Converted to: {json.dumps(converted_message, indent=2)}")
                converted_messages.append(converted_message)
            elif message.get("type") == "function_call_output":
                print(f"[DEBUG] Message type: function_call_output (already Response API format)")
                # Already in Response API format - ensure it has an id
                if "id" not in message:
                    message = message.copy()
                    message["id"] = f"msg_{i}"
                print(f"[DEBUG] Using message as-is: {json.dumps(message, indent=2)}")
                converted_messages.append(message)
            elif message.get("role") == "assistant" and "tool_calls" in message:
                print(f"[DEBUG] Message type: assistant with tool_calls")
                ...
                print(f"[DEBUG] Message type: other (role={message.get('role')}, type={message.get('type')})")
                # For other message types, just ensure they have an id
                filtered_message = message.copy()
                # Ensure it has an id
                if "id" not in filtered_message:
                    filtered_message["id"] = f"msg_{i}"
                print(f"[DEBUG] Message with id: {json.dumps(filtered_message, indent=2)}")
                converted_messages.append(filtered_message)

        print(f"\n[DEBUG] ===== convert_messages_to_response_api_format END =====")
        print(f"[DEBUG] Output messages count: {len(converted_messages)}")
        print(f"[DEBUG] Full output messages: {json.dumps(converted_messages, indent=2)}\n")
