...
class MessageConverter:
    ...
    def to_response_api_format(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Convert messages to OpenAI Response API format.
        
        Args:
            messages: Messages in any supported format
            
        Returns:
            Messages in Response API format
        """
        converted = []
        ...
        for msg in messages:
            ...
            if "tool_calls" in msg and msg["tool_calls"]:
                # Response API uses function_call (singular)
                first_call = msg["tool_calls"][0]
                func = first_call.get("function", {})
                response_msg["function_call"] = {
                    "name": func.get("name", ""),
                    "arguments": func.get("arguments", "{}")
                }
            
            converted.append(response_msg)
