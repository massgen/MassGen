# -*- coding: utf-8 -*-
"""
MCP client implementation for connecting to MCP servers. This module provides enhanced MCP client functionality to connect
with MCP servers and integrate external tools and resources into the MassGen workflow.
"""
import asyncio
import os
import re
...
class MCPClient:
        ...
        async with self._connection_lock:
            ...
            if self.status_callback:
                await self.status_callback(
                    ...
                    {
                        "server": self.name,
                        "message": f"Connecting to MCP server: {self.name}",
                    },
                )
    ...
    def _create_transport_context(self):
        ...
        if transport_type == "stdio":
            ...
            try:
                        ...
                        if isinstance(v, str) and "${" in v:
                            ...
                            def _rep_hdr(match):
                                ...
                                if val is None or val.strip() == "":
                                    raise ValueError(f"Required environment variable '{var_name}' is not set for HTTP header '{k}'")
                                return val
                ...
                logger.warning(f"Environment substitution failed for HTTP transport on {self.name}: {e}")
                raise
    ...
    async def _run_manager(self) -> None:
        ...
        try:
                ...
                async with ClientSession(read, write, read_timeout_seconds=session_timeout_timedelta) as session:
                    ...
                    if self.status_callback:
                        await self.status_callback(
                            ...
                            {
                                "server": self.name,
                                "message": f"✅ MCP server '{self.name}' connected successfully!",
            ...
            # Send error status if callback is available
            if self.status_callback:
                await self.status_callback(
                    "error",
                    {
                        "server": self.name,
                        "message": f"Failed to connect to MCP server '{self.name}': {e}",
                        "error": str(e),
                    },
                )
    ...
    async def _discover_capabilities_official(self) -> None:
        ...
        try:
            ...
            if any(
                ...
                logger.error(
                    f"Unexpected error listing resources for {self.name}: {e}",
                    exc_info=True,
                )
        ...
        try:
            ...
            if any(
                ...
                # Preserve original error context
                logger.error(
                    f"Unexpected error listing prompts for {self.name}: {e}",
                    exc_info=True,
                )
    ...
    async def call_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            if self.status_callback:
                await self.status_callback(
                    "tool_call_success",
                    {
                        "server": self.name,
                        "tool": tool_name,
                        "message": f"Tool '{tool_name}' executed successfully on server '{self.name}'",
                    },
                )
            ...
            # Send tool call timeout status if callback is available
            if self.status_callback:
                await self.status_callback(
                    "tool_call_timeout",
                    {
                        "server": self.name,
                        "tool": tool_name,
                        "message": f"Tool '{tool_name}' timed out after {self.timeout_seconds} seconds",
                        "timeout": self.timeout_seconds,
                    },
                )
        ...
        async with self._connection_lock:
            ...
            if self.status_callback:
                await self.status_callback(
                    "setup_start",
                    {
                        "message": f"Setting up MCP sessions with {len(self.server_configs)} servers",
    ...
    async def _connect_all(self) -> None:
        ...
        if self.status_callback:
            await self.status_callback(
                "setup_complete",
                {
                    "message": f"MCP setup complete: {len(self.clients)} servers connected",
