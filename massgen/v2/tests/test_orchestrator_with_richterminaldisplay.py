#!/usr/bin/env python3
"""
Test orchestrator with RichTerminalDisplay during actual task execution.
"""
...
async def test_orchestrator_with_richterminaldisplay():
    """Test orchestrator functionality with RichTerminalDisplay during real task execution."""
    
    # Disable logging to suppress INFO messages from orchestrator
    logging.getLogger('massgen.v2.orchestrator').setLevel(logging.WARNING)
    logging.getLogger('openai').setLevel(logging.WARNING)
    logging.getLogger('httpx').setLevel(logging.WARNING)
    
    print("🎭 Testing Orchestrator with RichTerminalDisplay Integration")
    print("=" * 60)
    ...
    orchestrator_config = OrchestratorConfig(
        orchestrator_id="richterminaldisplay_test_orchestrator",
    ...
    print(f"🖥️  Display integrated with orchestrator for real-time updates")
    ...
    print(f"📝 Using orchestrator with integrated RichTerminalDisplay...")
    print()
    ...
    try:
        ...
        async for chunk in orchestrator.chat(messages):
            if chunk.type == "content":
                # Skip printing individual agent messages since they're handled by the display
                if not (chunk.source and chunk.source in ["math_expert", "science_expert", "writing_expert"]):
    ...
    print("✅ RichTerminalDisplay integration test completed")
    ...
    print(f"  - RichTerminalDisplay: ✅ Working correctly")
    ...
    print("✅ RichTerminalDisplay is working correctly during task execution")


if __name__ == "__main__":
    asyncio.run(test_orchestrator_with_richterminaldisplay())