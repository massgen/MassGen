...
def _setup_agent_from_config(config: Dict[str, Any]) -> ConversableAgent:
    ...
    if agent_type == "assistant":
        ...
        raise ValueError(
            f"Unsupported AG2 agent type: {agent_type}. "
            "Use 'assistant' or 'conversable' for ag2 agents."
        )
...
class AG2Adapter(AgentAdapter):
    ...
    def __init__(self, **kwargs):
        ...
        if not self.agent_config and not self.group_config:
            raise ValueError(
                "Backend configuration must contain either 'agent_config' for single agent "
                "or 'group_config' for GroupChat."
            )
